(function($){
	$(function(){
                $.get(home_url + "/?r=ajax/math_worksheet/get", {lid: $('#sel-sublevelsSelectBoxIt').attr('data-selected')}, function (data) {
                        var worksheets = JSON.parse(data);
                        var cid = $('#cid').val();
                        var array = [];
                        ...
                        for(var i=0 ; i<array.length ;i++) {
                            if(array[i] == cid){
                                if(i == array.length-1) {
                                    ...
                                    $.each(worksheets, function (j, v) {
                                        if(v.sid == array[i+1]) {
                                            $('#linked-sheet').val(v.name);
                                        }
                                    });
                                }
                            };
		...
		$("#check-word").click(function(){
                    var check = $("#id-assign").val();
			if($("#sel-dictionary").val() == ""){alert("Please select a Dictionary1111!");return false;}
			var tthis = $(this);
		...
		$("#filter-levels").on("option-click", function(){
			...
			$_sl.append($("#_sl" + _l).html()).data("selectBox-selectBoxIt").refresh();
                ...
                $("#filter-subject").on("option-click", function(){
			var _l = $(this).val();
                        
			var $_sl = $("#filter-lesson");
			$_sl.find("option:not(:first-child)").remove();
			//$_sl.append($("#_sl" + _l).html()).data("selectBox-selectBoxIt").refresh();
                        if(_l!=""){
                           
                            $_sl.append($("#_sl" + _l).html()).data("selectBox-selectBoxIt").refresh().selectOption($_sl.attr("data-selected"));
                        }
		});
                $("#filter-subject").trigger("option-click");
		...
		if(typeof _PRELOAD !== "undefined"){
			if(_PRELOAD){
                ...
                // click button view description file
