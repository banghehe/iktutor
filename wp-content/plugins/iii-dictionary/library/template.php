...
?>
				...
				<div class="container" style="padding: 0;"> <!-- add div container only content -->
...
function ik_set_route($home_template)
{
	...
	if(is_admin_panel() && $layout != 'ajax')
	{
		...
		if(!$is_mw_super_admin && !$is_mw_admin && !$is_wp_admin && is_user_logged_in()) {
			//die();
	...
	if(!is_admin_panel() && is_user_logged_in()){
		$is_mw_super_admin = is_mw_super_admin();
		$is_mw_admin = is_mw_admin();
		$is_wp_admin = is_wp_admin();
		if( $is_mw_super_admin && $is_mw_admin && $is_wp_admin ) {
			ik_enqueue_messages(__('Wrong site for Login.', 'iii-dictionary'), 'error');
			wp_logout();
		}
	}
