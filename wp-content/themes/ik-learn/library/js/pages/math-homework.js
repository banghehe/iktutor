    ...
    $(function () {
        ...
        $("#step-nav").find("li").click(function () {
            ...
            if ($(this).hasClass('last-step')) {
                document.getElementById("input-answer").placeholder = "TYPE ANSWER HERE";
            ...
            if ($(this).hasClass("active")) {
                ...
                $('.word-prob-video').hide();
                $('.word-prob-video').trigger('pause');
                $('.word-prob-video').prop("currentTime", 0);
                $("#word-prob-video-q" + $(this).attr("data-n")).fadeIn(400);
            }
        ...
        $(".play_button").click(function () {
            ...
            $('#word-prob-video-' + idnew).trigger('play');
            $('.play_button').hide();
            $('#button-pause-video-' + idnew).show();
        });
        $(".pause_button").click(function () {
            var idnew = $(this).attr('data-id');
            $('#word-prob-video-' + idnew).trigger('pause');
            $('#word-prob-video-' + idnew).prop("currentTime", 0);
            $('.pause_button').hide();
            $('#button-play-video-' + idnew).show();
        });
        ...
        $("#submit-homework").click(function () {
            $("#submit-homework-modal").modal("show");
        });
        $("#step-nav, #qbox-step-nav").find("li:first").click();

        $('#btn-next-worksheet').click(function () {
            $('#site-messages-modal').modal('hide');
        });
        $("#close-modal-success").click(function () {
            $("#site-messages-modal").modal("hide");
        });
        $("#close-modal-homework").click(function () {
            $("#submit-homework-modal").modal("hide")
        });
        ...
        if (count_q == 2) {
            $('.question-box-block').css('padding-top', '120px');
        }
        $('#btn-next-practive-type1').click(function (e) {
//                    e.preventDefault();
            var array = new Array();
            $(".s1").each(function (index) {
                var string = $(this).attr("name");
                var number1 = string.split("[]");
                var number = number1[0];
                var ans = $(this).val();
//                        var question = $(this).attr("data-answer");
                var item = number + "[s" + index + "]:" + ans;
                array.push(item);
            });
            $('#data-ans-js').val(array);
            $('#data-type-assign-1').val(1);
        });
    });
})(jQuery);