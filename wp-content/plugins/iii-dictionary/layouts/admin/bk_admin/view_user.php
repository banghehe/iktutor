...
//	$user = new stdClass;
	...
	$is_registered_math_teacher = is_mw_registered_teacher($_GET['cid'],1);
	$is_registered_english_teacher = is_mw_registered_teacher($_GET['cid']);
	$is_qualified_math_teacher = is_mw_qualified_teacher($_GET['cid'],1);
	$is_qualified_english_teacher = is_mw_qualified_teacher($_GET['cid']);
	...
	if(isset($_POST['change-role'])) {
		...
		if($role == 'mw_qualified_teacher') {
			$user->add_role($role);
                        $user->remove_role('mw_qualified_math_teacher');
		}
		if($role == 'mw_qualified_math_teacher') {
			$user->add_role($role);
                        $user->remove_role('mw_qualified_teacher');
		}
                if($role == 'mw_qualified_both_teacher'){
                ...
                if($role == 'mw_registered_both_teacher'){
		...
		if($role=='mw_registered_math_teacher') {
                        $user->add_role('mw_registered_math_teacher');
			$user->remove_role('mw_qualified_math_teacher');
			$user->remove_role('mw_qualified_teacher');
			$user->remove_role('mw_registered_teacher');
                        delete_user_meta($user->ID, 'teacher_agreement_ver');
                        $agreement_update_date = mw_get_option('agreement-update-date');
                        update_user_meta($user->ID, 'math_teacher_agreement_ver', $agreement_update_date);

			// delete test results of the user so that the user can take test again
                        // math
			$group_id = mw_get_option('teacher-math-test-group');
		...
		if($role=='mw_registered_teacher') {
                        $user->add_role('mw_registered_teacher');
			$user->remove_role('mw_qualified_teacher');
                        $user->remove_role('mw_qualified_math_teacher');
			$user->remove_role('mw_registered_math_teacher');
                        delete_user_meta($user->ID, 'math_teacher_agreement_ver');
                        $agreement_update_date = mw_get_option('agreement-update-date');
                        update_user_meta($user->ID, 'teacher_agreement_ver', $agreement_update_date);
...
<?php get_dict_page_title('User Detail', 'admin-page') ?>
	...
	<form id="main-form" method="post" action="<?php echo home_url() . '/?r=view-user&amp;cid=' . $_GET['cid'] ?>">
			...
			<div class="col-xs-12">
				<table class="table table-striped table-style3">
					...
					<tr>
						<td>User Type:</td>
						<td>
							<?php if($is_qualified_english_teacher==true && $is_qualified_math_teacher==true) : ?>
								Qualified Both Teacher
							<?php else : if($is_qualified_math_teacher) : ?>
								Qualified Math Teacher
							<?php else : if($is_qualified_english_teacher) : ?>
                                                                Qualified English Teacher
							<?php else : if($is_registered_math_teacher==true && $is_registered_english_teacher==true ) : ?>
                                                                Registered Both Teacher
							<?php else : if($is_registered_math_teacher) : ?>
								Registered Math Teacher
							<?php else : if($is_registered_english_teacher) : ?>
		...
		<?php // teacher qualification test result
			if($is_registered_english_teacher ||$is_registered_math_teacher || $is_qualified_english_teacher|| $is_qualified_math_teacher) :
				...
				<div class="col-sm-4">
					<div class="form-group">
						<select class="select-box-it form-control" name="new-role">
							<option value="mw_registered_teacher"<?php echo $is_registered_english_teacher ? ' selected' : '' ?>>Registered English Teacher</option>
							<option value="mw_registered_math_teacher"<?php echo $is_registered_math_teacher ? ' selected' : '' ?>>Registered Math Teacher</option>
							<option value="mw_registered_both_teacher"<?php echo ($is_registered_english_teacher==TRUE && $is_registered_math_teacher==TRUE) ? ' selected' : '' ?>>Registered Both Teacher</option>
							<option value="mw_qualified_teacher"<?php echo $is_qualified_english_teacher ? ' selected' : '' ?>>Qualified English Teacher</option>
							<option value="mw_qualified_math_teacher"<?php echo $is_qualified_math_teacher ? ' selected' : '' ?>>Qualified Math Teacher</option>
							<option value="mw_qualified_both_teacher"<?php echo ($is_qualified_english_teacher==TRUE && $is_qualified_math_teacher==True)? ' selected' : '' ?>>Qualified Both Teacher</option>
						</select>
					</div>
				</div>
				<div class="col-sm-3">
