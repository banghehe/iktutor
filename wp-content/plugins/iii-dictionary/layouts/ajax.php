...
if ($task == 'get_my_lesson') {
    ...
    if ($search !== NULL) {
        $query .= 'WHERE ls.created_by=' . $user . ' AND ls.type Is null AND ls.name LIKE "%' . $search . '%" ORDER BY ls.created_on DESC';
        $gid_query .= ' WHERE ls.type="Public Lesson" AND  ls.created_by=' . $user . ' AND ls.name LIKE "%' . $search . '%" ORDER BY ls.created_on DESC';
    } else if ($sort !== null) {
        if ($sort == "upward") {
            $query .= 'WHERE  ls.created_by=' . $user . ' AND ls.type Is null ORDER BY ls.name ASC';
            $gid_query .= ' WHERE ls.type="Public Lesson" AND  ls.created_by=' . $user . ' ORDER BY ls.name ASC';
        } else if ($sort == "downward") {
            $query .= 'WHERE  ls.created_by=' . $user . ' AND ls.type Is null ORDER BY ls.name DESC';
            $gid_query .= ' WHERE ls.type="Public Lesson" AND  ls.created_by=' . $user . ' ORDER BY ls.name DESC';
        }
    } else {
        $query .= 'WHERE  ls.created_by=' . $user . ' AND ls.type Is null ORDER BY ls.created_on DESC';
        $gid_query .= ' WHERE ls.type="Public Lesson" AND  ls.created_by=' . $user . ' ORDER BY ls.created_on DESC';
    }
...
if ($task == 'get_my_subject') {
    ...
    if ($search !== NULL) {
        $query .= 'WHERE  ls.created_by=' . $user . ' AND ls.type Is null AND ls.name LIKE "%' . $search . '%" ORDER BY ls.created_on DESC';
        $gid_query .= ' WHERE ls.type="Public Subject" AND  ls.created_by=' . $user . ' AND ls.name LIKE "%' . $search . '%" ORDER BY ls.created_on DESC';
    } else if ($sort !== null) {
        if ($sort == "upward") {
            $query .= 'WHERE  ls.created_by=' . $user . ' AND ls.type Is null ORDER BY ls.name ASC';
            $gid_query .= ' WHERE ls.type="Public Subject" AND  ls.created_by=' . $user . ' ORDER BY ls.name ASC';
        } else if ($sort == "downward") {
            $query .= 'WHERE  ls.created_by=' . $user . ' AND ls.type Is null ORDER BY ls.name DESC';
            $gid_query .= ' WHERE ls.type="Public Subject" AND  ls.created_by=' . $user . ' ORDER BY ls.name DESC';
        }
    } else {
        $query .= 'WHERE  ls.created_by=' . $user . ' ORDER BY AND ls.type Is null AND ls.created_on DESC';
        $gid_query .= ' WHERE ls.type="Public Subject" AND  ls.created_by=' . $user . ' ORDER BY ls.created_on DESC';
    }
    $gid = $wpdb->get_results($gid_query);
    //var_dump($query);die;
    // var_dump($result);die;
...
if ($task == 'get_list_course') {
    ...
    $query = "SELECT g.special_group,g.id, g.name, g.created_on, g.created_by AS uid, gt.name AS group_type, price,class_type_id,content,detail,ct.name AS class_name,ordering 
...
if ($task == 'get_worksheet_course') {
    ...
    $query = "SELECT ds.sheet_name,ds.id AS sheet_id, gd.group_id FROM " . $wpdb->prefix . "dict_groups AS g "
...
if ($task == "add_list_subject_lesson") {
    ...
    if ($prerequisite != null) {
        if ($sub != null) {
            foreach ($sub as $v) {
                $order1 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_subject WHERE type='Public Subject' AND prerequisite is not null AND id != $v");
                $order2 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_subject WHERE type is null AND prerequisite is not null AND id != $v");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_subject SET prerequisite=$order1 WHERE id=$v AND type='Public Subject'");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_subject SET prerequisite=$order2 WHERE id=$v AND type is null");
            }
        }
        if ($id != null) {

            foreach ($id as $v) {
                $order1 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_lesson WHERE type='Public Lesson' AND prerequisite is not null AND id != $v");
                $order2 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_lesson WHERE type is null AND prerequisite is not null AND id != $v");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_lesson SET prerequisite=$order1 WHERE id=$v AND type='Public Lesson'");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_lesson SET prerequisite=$order2 WHERE id=$v AND type is null");
            }
        }
    } else {
        ...
        if ($sub != null) {
            foreach ($sub as $v) {
                $order1 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_subject WHERE type='Public Subject' AND no_prerequisite is not null AND id != $v");
                $order2 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_subject WHERE type is null AND no_prerequisite is not null AND id != $v");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_subject SET no_prerequisite=$order1 WHERE id=$v AND type='Public Subject'");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_subject SET no_prerequisite=$order2 WHERE id=$v AND type is null");
            }
        }
        if ($id != null) {

            foreach ($id as $v) {
                $order1 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_lesson WHERE type='Public Lesson' AND no_prerequisite is not null AND id != $v");
                $order2 = $wpdb->get_var("SELECT COUNT(id) FROM " . $wpdb->prefix . "dict_my_lesson WHERE type is null AND no_prerequisite is not null AND id != $v");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_lesson SET no_prerequisite=$order1 WHERE id=$v AND type='Public Lesson'");
                $wpdb->query("UPDATE " . $wpdb->prefix . "dict_my_lesson SET no_prerequisite=$order2 WHERE id=$v AND type is null");
            }
        }
    }
    die;
...
if ($task == 'get_list_subject_lesson') {
    ...
    if ($priority == 1) {
        $query2 = "SELECT * FROM " . $wpdb->prefix . "dict_my_subject WHERE type='Public Subject' AND  prerequisite is not null ORDER BY prerequisite ASC";
        $query = "SELECT * FROM " . $wpdb->prefix . "dict_my_lesson WHERE type='Public Lesson' AND  prerequisite is not null ORDER BY prerequisite ASC";
        $query3 = "SELECT * FROM " . $wpdb->prefix . "dict_my_lesson WHERE type is null AND  prerequisite is not null ORDER BY prerequisite ASC";
        $query4 = "SELECT * FROM " . $wpdb->prefix . "dict_my_subject WHERE type is null AND  prerequisite is not null ORDER BY prerequisite ASC";
    } else {
        $query2 = "SELECT * FROM " . $wpdb->prefix . "dict_my_subject WHERE type='Public Subject' AND  no_prerequisite is not null ORDER BY no_prerequisite ASC";
        $query = "SELECT * FROM " . $wpdb->prefix . "dict_my_lesson WHERE type='Public Lesson' AND no_prerequisite is not null  ORDER BY no_prerequisite ASC";
        $query3 = "SELECT * FROM " . $wpdb->prefix . "dict_my_lesson WHERE type is null AND no_prerequisite is not null ORDER BY no_prerequisite ASC";
        $query4 = "SELECT * FROM " . $wpdb->prefix . "dict_my_subject WHERE type is null AND no_prerequisite is not null  ORDER BY no_prerequisite ASC";
    }
...
if ($task == "login_account") {
    ...
    if($user){
        if ( in_array( 'administrator', $user->roles ) ) {
            $error = true;
        }else if( in_array( 'mw_super_admin', $user->roles ) ){
            $error = true;
        }else if( in_array( 'mw_admin', $user->roles ) ){
            $error = true;
        }
    }
    $creds['user_login'] = $user_name;
...
if ($task == "forgotpassword") {
    ...
    $has_err = false;
    if(empty($email)) {
        //echo __('Invalid email. Please enter the email you registered with.', 'iii-dictionary');
        $has_err = true;
    }
    ...
    if(!$has_err)
    {
        ...
        if ( $message && !wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
            echo __('The e-mail could not be sent.', 'iii-dictionary') . "<br>\n" . __('Possible reason: your host may have disabled the mail() function.', 'iii-dictionary');
        }
        else {
            echo __('Please check your email for instructions on how to reset your password. Please check your spam folder in case you do not receive the email.', 'iii-dictionary');
        }
    }else{
        echo __('Invalid email. Please enter the email you registered with.', 'iii-dictionary');
    }
