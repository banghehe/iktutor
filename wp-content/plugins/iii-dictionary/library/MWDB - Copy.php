...
class MWDB {
	...
	public static function get_sheets($filter, $active_only = false, $admin_panel = false)
	{
		...
		$items_per_page = isset($filter['items_per_page']) ? $filter['items_per_page'] : 99999999;

		// get total row
		$total = $wpdb->get_col($query);

		$columns = 't.*, gr.name AS grade, d.id as did, d.name, hal.name AS assignment, ht.name AS homework_type, u.display_name';
...
//		var_dump($query);die;
	...
	public static function get_received_private_message($id)
	{
		...
		$message = $wpdb->get_row(
			'SELECT m.subject, m.message, sender_id, u.user_login AS sender_login, u.display_name AS sender_name, status
	...
	public static function get_sent_private_message($id)
	{
		...
		$message = $wpdb->get_row(
			'SELECT m.subject, m.message, recipient_id, u.user_login AS recipient_login, u.display_name AS recipient_name
