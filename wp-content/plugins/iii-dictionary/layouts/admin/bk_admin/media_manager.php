...
//                var_dump($_POST['s_main_folder']);
		...
		if(!$has_err) {
			...
			$dir .= (!empty($data['sub-path'])) ? '/' . $data['sub-path']  : '';
                        $img_path = '/'.$dir;
                        $files = scandir($dir);
                        $count = count($files); //Don't do calculations in your loop, this is really slow

                        for ($i=2; $i<$count; $i++)
                        {
                            $num = $files[$i];
                            echo '<a  href="'.$img_path.'/'.$num.'" download><img class="img-list-adm" src="'.$img_path.'/'.$num.'" alt="It is file test not image" width="260" height="192" ></a>';
                        }
		}
        }
        if(isset($_GET['file'])){
				...
				<div class="form-group">
					...
					<button type="submit" name="upload-media" class="btn btn-default orange form-control"><span class="icon-plus"></span>Upload</button>
                        ...
                        <div class="col-sm-2 col-sm-offset-10">
                                <button type="submit" name="download-media" class="btn btn-default orange form-control"></span>Download All</button>
			</div>
						...
						<div class="col-xs-12">
							<div class="" style="max-height: 400px;overflow: auto;">
								<table class="table table-striped table-condensed ik-table1 vertical-middle text-center ">
									...
									<tbody id="omg-media-table"><?php MWHtml::list_folder_file($__dir, $__parent, $__root, $__sfile,$seach) ?></tbody>
								</table>
							</div>
						</div>
					</div>
